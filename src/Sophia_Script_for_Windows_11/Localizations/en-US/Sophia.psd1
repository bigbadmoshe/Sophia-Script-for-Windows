...
UnsupportedOSBuild                        = Script supports Windows 11 25H2 and higher. You're using {0} {1}. Run correct script based on Windows edition.
...
ReinstallWindowsWarning                   = Reinstall Windows using only a genuine ISO image.
Win10TweakerWarning                       = Windows has been infected with a trojan via a Win 10 Tweaker backdoor.
TweakerWarning                            = Windows stability may have been compromised by using {0}.
...
ControlledFolderAccessEnabledWarning      = You have Сontrolled folder access enabled. Please disable it and run script again.
...
CopilotPCSupport                          = Your PC has no a built-in AI accelerator (NPU), so there's no Windows AI related functions support. No need to disable anything using Windows policies. Recall function and Copilot application will only be removed.
