...
UnsupportedArchitecture                   = Vous utilisez un processeur basé sur l'architecture "{0}". Ce script ne prend en charge que les processeurs basés sur l'architecture Arm. Téléchargez et exécutez la version du script adaptée à votre architecture.
UnsupportedOSBuild                        = Le script ne supporte que Windows 11 25H2. Vous utilisez {0} {1}. Exécutez le script approprié en fonction de l'édition de Windows.
...
CopilotPCSupport                          = Votre PC ne dispose pas d'un NPU (accélérateur IA) intégré, il ne prend donc pas en charge les fonctions liées à l'IA Windows. Il n'est pas nécessaire de désactiver quoi que ce soit à l'aide des stratégies GPO. Seules la fonction de rappel et l'application Copilot seront supprimées.
